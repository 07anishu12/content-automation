...
def call_gemini(system_prompt, prompt, max_tokens=4000):
    return llm.call_gemini(system_prompt, prompt, max_tokens)
...
if carousel_json_str:
    ...
    carousel_data = JsonHelper.parse_llm_json(carousel_json_str)
    if carousel_data:
        try:
            ...
            print(f"Error saving Carousel JSON: {e}")
...
if infographic_json_str:
    from core.utils.json_helper import JsonHelper
    infographic_data = JsonHelper.parse_llm_json(infographic_json_str)
    if infographic_data:
        try:
            ...
            print(f"Error saving Infographic JSON: {e}")
